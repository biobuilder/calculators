...
mu = mu_Sun;
Rp = Rp_Ker; % periapsis 
Ra = Ra_Ker*5; % apoapsis
inc = inc_Ker+5*pi()/180; % inclination (rad)
argp = argp_Ker+5*pi()/180; % argument of periapsis (rad)
lonasc = lonasc_Ker+5*pi()/180; % longitude of ascending node (rad)
...
% find the index of the orbital point closest to this vector 
minRa = 1;
minRaIndex = 1;
minRd = 1;
minRdIndex = 1;
...
legend('orbit','periapsis','apoapsis','ascending node','descending node')
